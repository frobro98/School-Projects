...
{
	static TerrainObjectManager tmInstance;
...
void TerrainObjectManager::loadTerrain(const char* const terrainKey, const char* const heightmapFile, const float& sideLength, const float& maxHeight, const float& terrainHeight, const int& repeatU, const int& repeatV, const char* const textureKey)
{
	instance().createTerrainModel(terrainKey, heightmapFile, sideLength, maxHeight, terrainHeight, repeatU, repeatV, textureKey);
}

void TerrainObjectManager::createTerrainModel( const char* const terrainKey, const char* const heightmapFile, const float& sideLen, const float& maxHeight, const float& terrainHeight, const int &RepeatU, const int &RepeatV, const char* const textureKey)
{
	...
	const int numCellsPerRow = numVertsPerRow - 1;
	...
	const float heightRatio = maxHeight / 255.f;
	terrObj->cells = new Cell[numCells];
	terrObj->cellLength = cellLength;
	...
	const char* const file = SaveTerrainModel(terrainKey, verts, numVerts, triIndex, numTris);

	AssetManager::loadModel(file, terrainKey);

	delete[] file;

	terrObj->setupRender(terrainKey, textureKey);
...
const char* const TerrainObjectManager::SaveTerrainModel( const char* const terrainKey, const VertexStride_VUN* const pVerts, const int &num_verts, const TriangleIndex* const tlist, const int &num_tri)
{
	const char* const TerrainName = terrainKey;
	const int filenameSize = 128;
	char* const filename = new char[filenameSize];
	
	strcpy_s(filename, filenameSize, TerrainName);
	strcat_s(filename, filenameSize, ".azul");
	const char* const TerrainFilename = filename;
